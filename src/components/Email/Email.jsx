    ...
    return (!isMobile ? <>
        ...
        <>
            <div className={errors?.email ? m.errorStyleMobile : m.inputBoxMobile}>
                <input id='email' {...register("email", { onChange: (e) => { setEmail(e.target.value.trim()); }, required: { value: true, message: "Заповніть поле" }, pattern: { value: /^[a-zA-Z0-9._-]+@[a-zA-Z]+(\.[a-zA-Z]{1,4})+$/, message: `Поле має бути у форматі: email@example.com` } })} value={email} type="email" name="email" maxLength="100" />
